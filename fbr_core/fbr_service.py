...
class FBRPayloadBuilder:
    ...
    def build_sales_invoice_payload(self, invoice_data: Dict) -> Dict:
        ...
        invoice_type = "Debit Note" if invoice_data.get('is_debit_note', False) else "Sale Invoice"
        ...
        for item in invoice_data.get('items', []):
            ...
            item_entry = {
                ...
                "productDescription": item.get('description', item.get('item_name', '')),
                "rate": f"{tax_rate}%",
                "uoM": item.get('uom', ''),
...
class FBRValidator:
    ...
    def validate_invoice(self, invoice_data: Dict) -> Dict:
        ...
        if not invoice_data.get('tax_category'):
            errors.append("Tax Category (Seller Province) is required for FBR submission")
        ...
        if posting_date:
            ...
            if posting_date.date() != today:
                warnings.append(f"FBR requires posting date to be current date ({today})")
    ...
    def _validate_items(self, items: List[Dict]) -> List[str]:
        ...
        for idx, item in enumerate(items, 1):
            if not item.get('hs_code'):
                missing_hs_codes.append(f"Row {idx}: {item.get('item_name', 'Unknown')}")
            
            if not item.get('tax_rate'):
                missing_tax_rates.append(f"Row {idx}: {item.get('item_name', 'Unknown')}")
        
        if missing_hs_codes:
            errors.append(f"Following items are missing HS Codes: {', '.join(missing_hs_codes)}")
        
        if missing_tax_rates:
            errors.append(f"Following items are missing Tax Rates: {', '.join(missing_tax_rates)}")
...
class FBRSubmissionService:
    ...
    def submit_invoice(self, invoice_id: int, document_type: str = "Sales Invoice") -> Dict:
        ...
        try:
            ...
            error_msg = str(e)
            self._log_submission(invoice_id, document_type, {}, {"error": error_msg}, "Error")
            return {
                "success": False,
                "error": error_msg
            }
    
    def _submit_to_fbr_api(self, payload: Dict, invoice_id: int, document_type: str) -> Dict:
        ...
        try:
            ...
            return {
                "invoiceNumber": f"FBR{invoice_id}{datetime.now().strftime('%Y%m%d%H%M%S')}",
...
class FBRQueueManager:
    ...
    def add_to_queue(self, document_type: str, document_id: int, priority: int = 5) -> Dict:
        """Add document to FBR queue"""
        try:
            # Check if already in queue
            existing = self.session.query(FBRQueue).filter_by(
                document_type=document_type,
    ...
    def process_queue(self, limit: int = 50) -> Dict:
        ...
        try:
            # Get pending items
            queue_items = self.session.query(FBRQueue).filter(
                FBRQueue.status == "Pending",
                FBRQueue.retry_count < FBRQueue.max_retries
            ).order_by(
                FBRQueue.priority.desc(),
                FBRQueue.created_at.asc()
            ).limit(limit).all()
